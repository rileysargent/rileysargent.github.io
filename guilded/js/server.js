...
import {
    db, collection, addDoc, onSnapshot, query, where, doc, setDoc,
...
export class ServerController {
    ...
    static syncUserServers() {
        if (AppState.unsubscribeServers) AppState.unsubscribeServers();
        ...
        AppState.unsubscribeServers = onSnapshot(q, async (snapshot) => {
            AppState.servers = {};
            snapshot.forEach(d => {
                AppState.servers[d.id] = { id: d.id, ...d.data() };
            });
    ...
    static async joinServer(inviteCode) {
        ...
        const q = query(collection(db, 'servers'), where('inviteCode', '==', inviteCode.toUpperCase()));
        ...
        UIManager.showToast("Joined server!", 'success');
        return true;
    }
    ...
    static async createChannel(name, type) {
        ...
        await updateDoc(doc(db, 'servers', AppState.currentServer.id), {
            channels: arrayUnion({ id: safeId, name, type })
        });
    ...
    static async leaveServer() {
        ...
        const dd = Utils.$('#server-dropdown');
    ...
    static async saveServerSettings() {
        ...
        if (newName) {
            await updateDoc(doc(db, 'servers', AppState.currentServer.id), { name: newName });
        }
        UIManager.closeModal('modal-server-settings');
        UIManager.showToast("Server settings saved!", 'success');
    }
